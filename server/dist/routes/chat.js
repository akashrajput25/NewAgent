...
router.post('/:conversationId/stream', async (req, res, next) => {
    ...
    try {
        ...
        if (provider === 'anthropic') {
            ...
            const userMsg = image
                ? [{ type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: image.replace(/^data:image\/\w+;base64,/, '') } }, { type: 'text', text: message }]
                : message;
        ...
        const finalContent = fullThinking
            ...
            : sanitizedText;
        db.prepare('INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)').run(conversationId, 'assistant', finalContent);
        res.write('data: [DONE]\n\n');
        res.end();
    }
    catch (error) {
...
function getSystemMessage(personality) {
    const map = {
