...
router.post('/generate', async (req, res) => {
    ...
    const result = db.prepare('INSERT INTO generated_images (conversation_id, prompt, url, status) VALUES (?, ?, ?, ?)').run(conversationId ? Number(conversationId) : null, prompt, '', 'pending');
    ...
    try {
        ...
        db.prepare('UPDATE generated_images SET url = ?, status = ? WHERE id = ?').run(fileUrl, 'completed', id);
